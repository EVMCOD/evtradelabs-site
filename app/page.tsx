...
import HeroQuantMesh from "./_components/HeroQuantMesh";
...
function Kicker({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block text-[0.72rem] font-semibold tracking-[0.18em] uppercase text-[#7a6f63] mb-3">
      {children}
    </span>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1200px] mx-auto px-5">
        <div className="pb-14">
          <div className="hero-copy pt-8 lg:pt-16 max-w-[760px] xl:max-w-[680px]">
            ...
            <h1 className="hero-title text-white font-black text-[clamp(3.4rem,8vw,6.2rem)] leading-[0.9] tracking-[-0.065em] m-0 max-w-[12ch] text-balance">
              Herramientas y sistemas de trading para traders serios
            </h1>

            <p className="hero-body mt-7 text-[1.08rem] leading-[1.9] text-[#97a8c6] max-w-[60ch]">
            ...
            <div className="hero-actions flex flex-wrap gap-3 mt-9">
              <Link
                href="#productos"
                className="hero-cta-primary inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-[#2563eb] text-white font-semibold text-[0.92rem] shadow-[0_12px_30px_rgba(37,99,235,0.28)] hover:bg-[#1d4ed8] transition-colors duration-200"
              >
              ...
              <Link
                href="#comunidad"
                className="hero-cta-secondary inline-flex items-center px-6 py-3.5 rounded-full border border-white/14 bg-white/[0.02] text-[#c8d4eb] font-semibold text-[0.92rem] hover:border-white/28 hover:bg-white/[0.04] hover:text-white transition-all duration-200"
              >
                Unirse al Discord
              </Link>
            </div>

            <div className="hero-platforms flex flex-wrap gap-3 mt-9 text-[0.76rem] text-[#8da0c2]">
              {[
                "MetaTrader",
                "TradingView",
                "Risk tools",
                "Community + daily market brief",
              ].map((item) => (
            ...
            {/* Mini stats */}
            <div className="hero-stats flex flex-wrap gap-10 mt-11 pt-8 border-t border-white/10">
              {[
                { val: "Meta", label: "MetaTrader + TradingView" },
                { val: "Risk", label: "Tools and control" },
                { val: "Hub", label: "Community + brief" },
              ].map((s) => (
                <div key={s.val}>
                  <div className="text-white font-bold text-[1.34rem] tracking-tight">
                    {s.val}
                  </div>
                  <div className="text-[0.76rem] text-[#5e6f8e] mt-0.5 uppercase tracking-[0.14em]">
                    {s.label}
                  </div>
                </div>
              ))}
        ...
        <div className="mb-14 rounded-[30px] border border-white/10 bg-[linear-gradient(145deg,rgba(17,26,45,0.86),rgba(26,42,72,0.72))] p-8 md:p-10 shadow-[0_30px_90px_rgba(7,12,25,0.16)]">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            ...
            <div className="flex flex-col gap-3 min-w-[220px]">
              ...
              <Link href="#comunidad" className="inline-flex items-center justify-center rounded-full border border-white/12 bg-white/[0.03] px-6 py-3.5 text-[0.92rem] font-semibold text-[#c7d5eb] hover:border-white/20 hover:bg-white/[0.06] hover:text-white transition-all duration-200">
                Unirse al Discord
              </Link>
            </div>
          </div>
          ...
          <section className="reveal-card rounded-[28px] border border-white/10 bg-[rgba(17,26,45,0.52)] p-7 md:p-8 backdrop-blur-sm">
            ...
            <div className="mt-7 grid sm:grid-cols-2 gap-4">
              {availableNow.map((item) => (
                <article key={item.title} className="rounded-[22px] border border-white/8 bg-white/[0.035] p-5 transition-all duration-300 hover:-translate-y-1 hover:bg-white/[0.05]">
          ...
          <section className="reveal-card rounded-[28px] border border-white/10 bg-[rgba(17,26,45,0.38)] p-7 md:p-8 backdrop-blur-sm">
            ...
            <div className="mt-7 grid sm:grid-cols-2 gap-4">
              {ecosystemLayers.map((item) => (
                <article key={item.title} className="rounded-[22px] border border-white/8 bg-[rgba(255,255,255,0.025)] p-5 transition-all duration-300 hover:-translate-y-1 hover:bg-[rgba(255,255,255,0.04)]">
                  <h4 className="m-0 text-[1rem] font-semibold tracking-[-0.02em] text-white">{item.title}</h4>
                  <p className="mt-3 mb-0 text-[0.92rem] leading-[1.8] text-[#9fb2d4]">{item.body}</p>
                </article>
              ))}
    ...
    <section className="section-shell py-24 bg-transparent">
      <div className="w-full max-w-[1200px] mx-auto px-5">
        <div className="text-center mb-12">
          ...
          <h2 className="m-0 text-[clamp(1.95rem,4.2vw,3rem)] font-black leading-[1.03] tracking-[-0.05em] text-[#131313] max-w-[620px] mx-auto text-balance">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {steps.map((s) => (
            <div
              key={s.n}
              className="reveal-card p-6 border border-[#e2d9ce] rounded-2xl bg-[#fffdfa] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(27,31,39,0.05)]"
            >
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-[#edf3ff] text-[#1f6fff] font-black text-[0.85rem] mb-5">
                {s.n}
              </div>
              <h3 className="m-0 mb-2.5 font-bold text-[1rem] text-[#131313]">
                {s.title}
              </h3>
              <p className="m-0 text-[0.88rem] leading-[1.75] text-[#5f5a54]">
                {s.body}
              </p>
            </div>
          ))}
    ...
    <section id="comunidad" className="section-shell py-24 bg-transparent">
      <div className="w-full max-w-[1200px] mx-auto px-5">
        <div className="grid lg:grid-cols-2 gap-4">
          {/* Discord card */}
          <article className="reveal-card p-8 border border-white/10 rounded-3xl bg-[linear-gradient(145deg,rgba(18,30,52,0.88),rgba(25,40,68,0.74))] flex flex-col transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(7,12,25,0.18)]">
            ...
            <div>
              <Link
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full border border-white/12 text-white font-semibold text-[0.9rem] hover:bg-white/8 hover:text-white hover:border-white/20 transition-all duration-200"
              >
                <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
                  <path
                    d="M13.55 1.24A13.3 13.3 0 0 0 10.2.5c-.14.26-.3.6-.42.88a12.3 12.3 0 0 0-3.56 0A9.3 9.3 0 0 0 5.8.5 13.3 13.3 0 0 0 2.45 1.24C.35 4.35-.21 7.37.07 10.35A13.4 13.4 0 0 0 4.16 12c.33-.43.62-.9.87-1.39a8.7 8.7 0 0 1-1.36-.64l.33-.24a9.6 9.6 0 0 0 8 0l.33.24c-.43.25-.89.47-1.37.64.25.49.54.96.87 1.39A13.4 13.4 0 0 0 15.93 10.35C16.26 6.92 15.38 3.93 13.55 1.24zM5.34 8.5c-.77 0-1.41-.7-1.41-1.55s.62-1.56 1.41-1.56c.78 0 1.42.7 1.41 1.56 0 .85-.63 1.55-1.41 1.55zm5.32 0c-.77 0-1.41-.7-1.41-1.55s.63-1.56 1.41-1.56c.78 0 1.41.7 1.41 1.56 0 .85-.63 1.55-1.41 1.55z"
                    fill="currentColor"
                  />
                </svg>
                Únete al Discord
          ...
          <div className="grid gap-4">
            <article
              id="acceso"
              className="reveal-card p-8 rounded-3xl flex flex-col bg-gradient-to-br from-[#111827] to-[#1a2233] text-[#f3f6fd] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_28px_70px_rgba(17,24,39,0.22)]"
            >
